import { FeePaymentMethod } from "@aztec/aztec.js";
...
export abstract class EntrypointPayload {
	...
	protected constructor(
		...
		this.#nonce = nonce ? nonce : Fr.random();
	}
	 ...
	 * @returns The execution payload
	 */
	static fromAppExecution(
		functionCalls: FunctionCall[] | Tuple<FunctionCall, 4>,
		nonce: Fr
	) {
		if (functionCalls.length > APP_MAX_CALLS) {
			throw new Error(
				`Expected at most ${APP_MAX_CALLS} function calls, got ${functionCalls.length}`
			);
		}
		const paddedCalls = padArrayEnd(
			functionCalls,
			FunctionCall.empty(),
			APP_MAX_CALLS
		);
	...
	static async fromFeeOptions(sender: AztecAddress, feeOpts?: FeeOptions) {
		const calls =
			(await feeOpts?.paymentMethod.getFunctionCalls(feeOpts?.gasSettings)) ??
			[];
		const feePayer = await feeOpts?.paymentMethod.getFeePayer(
			feeOpts?.gasSettings
		);
